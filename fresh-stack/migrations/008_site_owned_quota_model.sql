-- Migration 008: Site-owned quota foundation
--
-- This migration adds the additive schema needed to move entitlements from
-- license/email ownership to canonical WordPress site ownership. It is
-- intentionally additive so the existing license-centric paths can keep
-- running during rollout.

-- -------------------------------------------------------------------------
-- 0. Align live schema drift with the current runtime
-- -------------------------------------------------------------------------
...
CREATE INDEX IF NOT EXISTS idx_usage_logs_license_id ON public.usage_logs(license_id);
...
BEGIN
  IF NOT EXISTS (
    ...
    WHERE constraint_name = 'fk_subscriptions_site'
...
-- -------------------------------------------------------------------------
-- 1. Canonical site identity columns
-- -------------------------------------------------------------------------
...
BEGIN
  IF NOT EXISTS (
    SELECT 1
    FROM information_schema.table_constraints
    WHERE constraint_name = 'fk_sites_owner_user'
  ...
  IF NOT EXISTS (
    SELECT 1
    FROM information_schema.table_constraints
    WHERE constraint_name = 'fk_sites_merged_into'
      AND table_name = 'sites'
  ) THEN
    ALTER TABLE public.sites
...
SET
  ...
  first_seen_at = COALESCE(first_seen_at, activated_at, NOW()),
...
CREATE INDEX IF NOT EXISTS idx_sites_wp_install_uuid ON public.sites(wp_install_uuid);
CREATE INDEX IF NOT EXISTS idx_sites_site_fingerprint ON public.sites(site_fingerprint);
CREATE INDEX IF NOT EXISTS idx_sites_canonical_domain ON public.sites(canonical_domain);
CREATE INDEX IF NOT EXISTS idx_sites_normalized_site_url ON public.sites(normalized_site_url);
CREATE INDEX IF NOT EXISTS idx_sites_merged_into_site_id ON public.sites(merged_into_site_id);

-- -------------------------------------------------------------------------
-- 2. Site-owned entitlement tables
-- -------------------------------------------------------------------------
...
FROM public.sites s
JOIN public.licenses l ON l.license_key = s.license_key
...
CREATE INDEX IF NOT EXISTS idx_site_subscriptions_site_id ON public.site_subscriptions(site_id);
CREATE INDEX IF NOT EXISTS idx_site_subscriptions_status ON public.site_subscriptions(status);
CREATE INDEX IF NOT EXISTS idx_site_subscriptions_customer_id ON public.site_subscriptions(stripe_customer_id);
...
CREATE INDEX IF NOT EXISTS idx_site_quotas_site_period ON public.site_quotas(site_id, quota_period_start DESC);

CREATE TABLE IF NOT EXISTS public.site_trials (
  id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
  site_id UUID NOT NULL REFERENCES public.sites(id) ON DELETE CASCADE,
  trial_type VARCHAR(50) NOT NULL DEFAULT 'initial',
  total_trial_credits INTEGER NOT NULL DEFAULT 3,
...
CREATE INDEX IF NOT EXISTS idx_site_trials_site_id ON public.site_trials(site_id);
CREATE UNIQUE INDEX IF NOT EXISTS uq_site_trials_active_initial
...
CREATE INDEX IF NOT EXISTS idx_usage_events_site_id_created_at ON public.usage_events(site_id, created_at DESC);
...
CREATE INDEX IF NOT EXISTS idx_site_audit_logs_site_id ON public.site_audit_logs(site_id, created_at DESC);
...
CREATE INDEX IF NOT EXISTS idx_site_merges_target_site_id ON public.site_merges(target_site_id, created_at DESC);

-- -------------------------------------------------------------------------
-- 3. Atomic quota and merge functions
-- -------------------------------------------------------------------------
...
CREATE OR REPLACE FUNCTION public.bbai_reserve_site_generation(
  ...
  p_trial_credits INTEGER DEFAULT 3
) RETURNS JSONB
LANGUAGE plpgsql
AS $$
DECLARE
  ...
  IF v_quota_mode = 'trial' THEN
    ...
    IF NOT FOUND THEN
      INSERT INTO public.site_trials (
        ...
        p_site_id,
        'initial',
        GREATEST(COALESCE(p_trial_credits, 3), 1),
        0,
        'active'
      )
